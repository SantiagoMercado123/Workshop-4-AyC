...
package com.mycompany.ayc2;
/*
 * Algorithms and Complexity                                  August 04, 2022
 * IST 4310-01
...
public class AyC2 {
	...
	{
            Cases(200000, 300, 1); //Best Case (200000 max amount of n integers, 300 repetitions).
	...
	// implementations:
        ...
        private static void Cases(int ii, int jj, int opti){ //(ii is the amount of integers, jj is the amount of repetitions, opti is the case we are evaluating).
            runtimes = new LinkedList<Integer>();
                ...
                if(opti == 1){
                    ...
                    factor = 3; //i grows times 3/2 (1.5) instead of 2 in order to get more data in worst and average case.
                }
                
                for (int i=32; i<ii; i=i*factor/2){ 
                    counter = 0;
                    long total = 0;
                    for(int j=0; j<jj; j++){
        ...
        private static void Count(){ //Counts each time a unique number appears in an array.
           ...
           actualsize=0;
                        int x;
                        int z=0;
			while (z<size){ // reads every integer in the list array.
			  x = list[z];
                          z++;
                          int pos = 0;
                          boolean keep = true;
